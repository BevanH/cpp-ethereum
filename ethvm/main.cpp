...
int main(int argc, char** argv)
{
    ...
    if (mode == Mode::Statistics)
    {
        ...
        for (auto const& c : {Instruction::SSTORE, Instruction::SLOAD, Instruction::CALL,
            ...
            if (counts[(byte)c].first != 0)
                cout << "  " << instructionInfo(c).name << " x " << counts[(byte)c].first << " ("
                     << counts[(byte)c].second << " gas)\n";
    }
    else if (mode == Mode::Trace)
